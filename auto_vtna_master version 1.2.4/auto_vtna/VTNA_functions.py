...
def VTNA_tT(dt_lists, dNS_lists, order_combination):
    ...
    raised to their respective order values in order_combination using the trapezoid rule.
    """

    # Small value to replace zeros
    eps = 1e-10

    # Replace zeros (and optionally negatives) safely
    # You can choose one of these depending on your preference:

    # Option A (recommended): only replace exact zeros
    safe_dNS_lists = np.where(dNS_lists == 0, eps, dNS_lists)

    # Option B (more robust): replace anything very small or <= 0
    # safe_dNS_lists = np.where(dNS_lists <= eps, eps, dNS_lists)

    # Make an order combination matrix
    order_array = np.array(order_combination)[:, np.newaxis, np.newaxis]

    # Raise concentrations to powers
    dNS_lists_updated = np.power(safe_dNS_lists, order_array)

    # Multiply species together and apply dt
...
def on_click(event,VTNA_selection,data,y_unit,size_scaler,fit_metric,deg,constraint,fixed_order_species=None,axis_species=None,show_legend=True,DP_scaler=1,linewidth_scaler=1):
    """
    ...
    Args:
        ...
        DP_scaler (float): Factor by which the size of the datapoints in the generated VTNA overlay plot is adjusted.
        linewidth_scaler (float): Factor by which the linewidth of the lines in the generated VTNA overlay plot is adjusted.
    """
    plt.ion()
    ...
    if event.button==3 or event.button==2:
        ...
        for i in experiments:
            ...
            ax2.plot(x_values,ys,marker='o',label=i,markersize=3*DP_scaler,linewidth=0.8*linewidth_scaler)
        ...
        if constraint=='monotonic':
            ...
            fit_plot,=ax2.plot(ts_2,ts_2*a,marker='',label='linear fit\nvia origin',linestyle='dotted',linewidth=1.5,color='black')
        else:
...
def same_excess_v2(data, selected_experiments, selected_species,
                  selected_product_species=None, deg=7, zero_clip_atol=0.0):
    """
    Same-excess for 2+ experiments with optional product-based alignment.
    - If selected_product_species is None: time-shift by matching initial REACTANT
      via t = f(reactant) on the standard (previous behaviour).
    - If selected_product_species is provided:
        1) shift each product curve so it starts at 0 (subtract its own initial),
        2) lift product curves by ΔR0 (= R0_standard - R0_exp),
        3) time-shift by matching initial (shifted) PRODUCT via t = f(product) on the standard.
    """
    # ---- Basic checks and deep copy ----
    if not isinstance(selected_experiments, (list, tuple)) or len(selected_experiments) < 2:
        err = "Error: Provide at least two experiments for same-excess analysis."
        print(err); return err

    data_same_excess = {}
    for exp in selected_experiments:
        if exp not in data:
            err = f"Error: '{exp}' is not a key in data."
            print(err); return err
        if selected_species not in data[exp].columns:
            err = f"Error: Experiment '{exp}' does not contain species '{selected_species}'."
            print(err); return err
        if selected_product_species is not None and selected_product_species not in data[exp].columns:
            err = f"Error: Experiment '{exp}' does not contain product '{selected_product_species}'."
            print(err); return err
        data_same_excess[exp] = copy.deepcopy(data[exp])

    # ---- Choose standard experiment (highest initial reactant) ----
    init_R = {exp: float(data_same_excess[exp][selected_species].iloc[0])
              for exp in selected_experiments}
    standard_exp = max(init_R, key=init_R.get)
    low_initial_conc_experiment = min(init_R, key=init_R.get)  # for compatibility
    time_label = data_same_excess[standard_exp].columns[0]

    # ---- NEW: Zero product at start, then apply vertical shift by ΔR0 ----
    if selected_product_species is not None:
        R0_std = init_R[standard_exp]
        for exp in selected_experiments:
            # 1) zero-at-start: subtract the experiment's initial product value
            y = np.asarray(data_same_excess[exp][selected_product_species], dtype=float)
            y0 = float(y[0])
            y = y - y0

            # (optional) clip tiny negatives due to noise
            if zero_clip_atol is not None and zero_clip_atol >= 0:
                y[y > -zero_clip_atol] = np.maximum(y[y > -zero_clip_atol], 0.0)

            # 2) apply ΔR0 lift
            delta = R0_std - init_R[exp]  # positive if exp is more dilute
            y = y + float(delta)

            # write back
            data_same_excess[exp][selected_product_species] = y

    # ---- Helpers ----
    def _monotone_increasing(arr):
        """Force weakly monotone increasing (e.g., product) via cumulative max."""
        a = np.asarray(arr, dtype=float)
        return np.maximum.accumulate(a)

    def _monotone_decreasing(arr):
        """Force weakly monotone decreasing (e.g., reactant) via cumulative min (from start)."""
        a = np.asarray(arr, dtype=float)
        return np.minimum.accumulate(a)

    # ---- Build mapping on the standard and apply shifts ----
    ts_std = np.asarray(data_same_excess[standard_exp][time_label], dtype=float)

    if selected_product_species is None:
        # === Reactant-based mapping (as before) ===
        ys_std = np.asarray(data_same_excess[standard_exp][selected_species], dtype=float)
        ys_std_mono = _monotone_decreasing(ys_std)

        # Domain for overlap check
        y_min_std, y_max_std = float(np.min(ys_std_mono)), float(np.max(ys_std_mono))

        # Build t = f(reactant): flip to ascending for interp
        x_for_interp = ys_std_mono[::-1]  # ascending
        t_for_interp = ts_std[::-1]

        for exp in selected_experiments:
            if exp == standard_exp:
                continue
            y0 = float(data_same_excess[exp][selected_species].iloc[0])
            if not (y_min_std <= y0 <= y_max_std):
                err = (f"Error: Initial {selected_species} of '{exp}' ({y0:g}) "
                       f"is outside the standard's range [{y_min_std:g}, {y_max_std:g}].")
                print(err); return err

            # Time when standard's reactant equals y0
            t_shift = float(np.interp(y0, x_for_interp, t_for_interp))
            data_same_excess[exp][time_label] = (
                np.asarray(data_same_excess[exp][time_label], dtype=float) + t_shift
            )
    else:
        # === Product-based mapping (using product AFTER zeroing and ΔR0) ===
        ps_std = np.asarray(data_same_excess[standard_exp][selected_product_species], dtype=float)
        ps_std_mono = _monotone_increasing(ps_std)

        p_min_std, p_max_std = float(np.min(ps_std_mono)), float(np.max(ps_std_mono))

        # Build t = f(product) via linear interpolation (ascending)
        x_for_interp = ps_std_mono
        t_for_interp = ts_std

        for exp in selected_experiments:
            if exp == standard_exp:
                continue
            # Initial product after zeroing & ΔR0
            p0 = float(data_same_excess[exp][selected_product_species].iloc[0])

            if not (p_min_std <= p0 <= p_max_std):
                err = (f"Error: Initial (shifted) {selected_product_species} of '{exp}' ({p0:g}) "
                       f"is outside the standard's product range [{p_min_std:g}, {p_max_std:g}].")
                print(err); return err

            # Time when standard's product equals p0
            t_shift = float(np.interp(p0, x_for_interp, t_for_interp))
            data_same_excess[exp][time_label] = (
                np.asarray(data_same_excess[exp][time_label], dtype=float) + t_shift
            )

    return data_same_excess, low_initial_conc_experiment
        ...
        for RM in VTNA_selection['RM'].keys():
            ...
            for i in range(len(table[t])-1):
                ...
                for NS in VTNA_selection["normalised_species"].keys():
                    ...
                    avg_val = (float(table[NS][i+1]) + float(table[NS][i])) / 2
                    # Avoid infinity when the average value is 0. 
                    avg_val = avg_val if avg_val > eps else eps
                    C = C * (avg_val ** order)
