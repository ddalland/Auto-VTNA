...
def check_kinetic_data(kinetic_data, bold_and_red=True, print_report=True):
    ...
    string with relevant information and warnings.

    The function checks that:
    - kinetic_data is a dictionary
    - each value in the dictionary is a pandas DataFrame
    - no DataFrame is empty
    - column titles are consistent across experiment DataFrames
    - there are enough columns for VTNA
    - all time/concentration values are numerical
    - no negative values are present
    - the time column is strictly monotonically increasing
    - column titles are strings and not numeric-like

    Expected input format:
        kinetic_data = pd.read_excel("file_name.xlsx", sheet_name=None)

    Alternatively, each experiment can be loaded from separate CSV files and collected into a dict.
    """
    import math
    import re
    import numbers
    ...
    def all_elements_not_convertible_to_float(my_list):
        def is_float_convertible(x):
            try:
                ...
                return False
        return all(not is_float_convertible(element) for element in my_list)
    ...
    report = (
        ...
        "--------------------------\n"
        "\033[1mCheck 1: Correct loading of kinetic data from Excel.\033[0m\n"
    )

    # Check that kinetic_data is a dictionary
    if not isinstance(kinetic_data, dict):
        report += (
            "\033[91mOBS:\033[0m The kinetic data is not loaded correctly. It should "
            "be a dictionary containing the different experiment datasets as dataframes.\n"
            "Make sure that the excel file contains one experiment dataset per\n"
            "sheet with column titles for time and concentration of reaction species and load "
            "the data like this: pd.read_excel('file_name.xlsx', sheet_name=None)\n"
        )
        report += (
            "\033[1mConclusion: the kinetic_data variable is not ready for use with the "
            "auto_vtna package.\033[0m"
        )
        if bold_and_red is False:
            report = remove_escape_sequences(report)
        if print_report:
            print(report)
        return report

    report += f"  {tick_symbol} The kinetic data is loaded as a dictionary.\n"

    # Check that all elements are pandas DataFrames
    if not all(isinstance(df, pd.DataFrame) for df in kinetic_data.values()):
        report += (
            "\033[91mOBS:\033[0m Some of the elements in the kinetic data dictionary are "
            "not pandas dataframe objects. This will need to be fixed.\n"
            "Make sure the excel file contains one experiment dataset per\n"
        ...
        report += (
            "\033[1mConclusion: the kinetic_data variable is not ready for use with "
    ...
    report += (
        f"  {tick_symbol} Each element associated a key in the kinetic data dictionary "
        f"is a pandas dataframe object.\n"
    )

    # Check that no DataFrames are empty
    has_empty_dataframes = any(df.empty for df in kinetic_data.values())
    if has_empty_dataframes:
        report += (
            "\033[91mOBS:\033[0m One or more experiment datasets are empty.\n"
            "Make sure that there are no empty sheets and that the excel file contains one "
            "experiment dataset per\nsheet with column titles for time and concentration of "
            "reaction species.\n"
        )
        report += (
            "\033[1mConclusion: the kinetic_data variable is not ready for use with the "
    ...
    for exp in kinetic_data.keys():
        cols = kinetic_data[exp].columns.tolist()
        column_titles.append(cols)
        no_columns.append(len(cols))
        columns.append(cols)

    # Check if only one experiment
    if no_experiments == 1:
        error = True
        report += (
            "\033[91mOBS:\033[0m The kinetic data only contains data from one "
            "experiment. This is not enough for VTNA.\n"
        )

    # Check column counts
    if len(no_columns) > 0 and all(i == no_columns[0] for i in no_columns):
        report += (
            f"  {tick_symbol} The dataframe element(s) in the kinetic data dictionary "
            f"all contain {no_columns[0]} columns.\n"
        )
        if no_columns[0] == 1:
            report += (
                "\033[91mOBS:\033[0m The dataframe elements in the kinetic data dictionary "
                "contain only one column. This is insufficient for VTNA.\n"
            )
            report += (
                "\033[1mConclusion: the kinetic data is not ready for use with the "
                "auto_vtna package.\033[0m"
            )
            if bold_and_red is False:
                report = remove_escape_sequences(report)
    ...
    # Check 2
    report += "\033[1mCheck 2: Column title consistency.\033[0m\n"

    if len(no_columns) > 0 and all(i == no_columns[0] for i in no_columns):
        column_titles_array = np.array(column_titles, dtype=object)
        are_equal = np.all(column_titles_array == column_titles_array[0, :], axis=0)
        n_cols = len(column_titles_array[0])

        if all(are_equal):
            report += f"  {tick_symbol} All experiment datasets have the same column titles.\n"
        else:
            error = True
            for i in range(n_cols):
                if not are_equal[i]:
                    report += (
                        f"\033[91mOBS:\033[0m The {num2words(i + 1, to='ordinal_num')} "
                        f"column title is different across experiments: "
                        f"{column_titles_array[:, i]} \n"
                    )
    else:
        error = True
        report += (
            "\033[91mOBS:\033[0m the number of columns in the dataframe objects in "
            "kinetic_data are not consistent.\nThe column titles are:\n"
        )
        for i in range(len(column_titles)):
            report += f"  - {list(kinetic_data.keys())[i]}: {column_titles[i]}\n"

    # Check 3
    counter = 0
    report += "\033[1mCheck 3: Numerical datapoints.\033[0m \n"

    for exp in kinetic_data.keys():
        df = kinetic_data[exp]
        for column in list(df.columns):
            numeric_col = pd.to_numeric(df[column], errors="coerce")
            num_column = numeric_col.notna().tolist()
            ...
            if all(num_column):
                ...
                error = True
                counter += 1
                bad_rows = [index + 2 for index, value in enumerate(num_column) if value is False]
                report += "\033[91mOBS:\033[0m"
                report += (
                    f' Row(s) number {", ".join(map(str, bad_rows))} in column "{column}" '
                    f'in dataset "{exp}" contains values which are not numerical. '
                    f"Needs to be fixed.\n"
                )

    if counter == 0:
        report += f"  {tick_symbol} Every experiment dataset contains only numerical values.\n"

    # Check 4
    counter = 0
    report += "\033[1mCheck 4: Positive datapoints.\033[0m \n"

    for exp in kinetic_data.keys():
        df = kinetic_data[exp]
        for column in list(df.columns):
            numeric_col = pd.to_numeric(df[column], errors="coerce")
            any_negative_values = (numeric_col < 0).fillna(False).tolist()
            ...
            if not any(any_negative_values):
                ...
                error = True
                counter += 1
                bad_rows = [index + 2 for index, value in enumerate(any_negative_values) if value]
                report += "\033[91mOBS:\033[0m"
                report += (
                    f' Row(s) number {", ".join(map(str, bad_rows))} in column "{column}" '
                    f'in dataset "{exp}" contains negative values. Needs to be fixed.\n'
                )

    if counter == 0:
        report += (
            f"  {tick_symbol} No datasets contain negative time or concentration values.\n"
        )

    # Check 5
    report += "\033[1mCheck 5: Time column monotonicity.\033[0m\n"
    counter = 0

    for exp in kinetic_data.keys():
        time_col_name = kinetic_data[exp].columns[0]
        time_array = pd.to_numeric(kinetic_data[exp][time_col_name], errors="coerce").to_numpy()

        # Only evaluate monotonicity if all values are numeric
        if np.any(pd.isna(time_array)):
            report += (
                f"\033[91mOBS:\033[0m The time values in dataset {exp} could not be fully "
                f"interpreted as numerical, so monotonicity could not be confirmed.\n"
            )
            error = True
            counter += 1
        elif not np.all(time_array[1:] > time_array[:-1]):
            report += (
                f"\033[91mOBS:\033[0m The time values in dataset {exp} are not strictly "
                f"monotonically increasing. Needs to be fixed.\n"
            )
            error = True
            counter += 1
    ...
    if counter == 0:
        report += (
            f"  {tick_symbol} The time column (first column) of the dataset in every sheet "
            f"is monotonically increasing.\n"
        )

    # Check 6
    report += f"\033[1mCheck 6: Time and concentration columns.\033[0m \n"
    check_for_floats = 0

    columns_flat = []
    for col_list in columns:
        columns_flat.extend(col_list)

    # Check if all column titles are strings
    if all(isinstance(i, str) for i in columns_flat):
        report += (
            f"  {tick_symbol} All dataframe elements in the kinetic data dictionary "
            f"have column titles which are strings.\n"
        )
    else:
        for exp_no in range(len(list(kinetic_data.keys()))):
            if all_elements_not_convertible_to_float(columns[exp_no]):
                ...
                check_for_floats = 1
                report += (
                    f"\033[91mOBS:\033[0m {count_convertible_to_float(columns[exp_no])} "
                    f"of the column titles of dataframe "
                    f"{list(kinetic_data.keys())[exp_no]} are numerical values.\n"
                )
                error = True

    if check_for_floats == 0:
        report += (
            f"  {tick_symbol} The column titles of every experiment dataset are all strings.\n"
        )

    time_column_titles = [i[0] for i in columns]

    if len(time_column_titles) > 0 and all(i == time_column_titles[0] for i in time_column_titles):
        time_column_titles_display = time_column_titles[0]
    else:
        time_column_titles_display = time_column_titles

    report += (
        f"\033[91mNB:\033[0m Make sure that the time columns are the first columns in "
        f"each dataframe in kinetic_data. Currently, the first column titles are: "
        f"{time_column_titles_display}.\n"
    )

    # Conclusion
    if error is None:
        report += (
            "\033[1mConclusion:\033[0m The kinetic data has been loaded correctly if "
            "the time axis label is correct."
        )
    else:
        report += (
            "\033[91m\033[1mConclusion:\033[0m The kinetic data has not been loaded "
            "correctly. Make sure that the kinetic data Excel spreadsheet is organised "
            "correctly and try again using kinetic_data=pd.read_excel('file_name.xlsx', "
            "sheet_name=None)."
        )
...
def plot_data(data, ylim=None, y_unit='M', t_unit=None, fig_size_scaler=1,plot_mode='together',
    legend_outside=False, DP_scaler=0.35,linewidth=0.7,show_legend=True):
    """
    ...
    if plot_mode=='together':
        ...
        class ScrollablePlot:
            def __init__(self, data_dict, ylim=None, y_unit='M', t_unit=None, 
                         fig_size_scaler=1,legend_outside=legend_outside,DP_scaler=0.35,linewidth=0.7,show_legend=True):
        ...
        fig_size_scaler=fig_size_scaler,legend_outside=legend_outside,DP_scaler=DP_scaler,linewidth=linewidth,show_legend=show_legend)
        plt.show()
    else:
...
def plot_data_together(data, species, ylim=None, y_unit='mM', fig_size_scaler=1, t_unit=None,
    ...
    fig, ax = plt.subplots(figsize=(7 * 0.75 * fig_size_scaler, 5 * 0.75 * fig_size_scaler))
    
    # Define a colormap for assigning different colors to each experiment
    base_colors = plt.get_cmap('tab10')  # A colormap with distinct colors for each experiment
    
    # Define a set of markers to be used for each species within an experiment
    markers = ['^', 's', 'x', 'o', '*', 'd',]  # Simple, distinguishable markers
    
    # Iterate through each experiment
    for idx, i in enumerate(data.keys()):
        time = data[i][t]
        
        # Use a distinct color for each experiment
        base_color = base_colors(idx % 10)
        
        # Iterate through each species in the experiment
        for j_idx, j in enumerate(data[i].keys()):
            if j == t or j not in species:
                continue
            
            # Use different markers for each species within the same experiment
            marker = markers[j_idx % len(markers)]
            
            # Plot the concentration profile with the generated marker and color
            ax.plot(time, data[i][j], label=f'{i}, {j}', color=base_color,
                    linestyle='-', marker=marker, markersize=6 * DP_scaler, linewidth=linewidth)
        
        ax.set_title('Concentration profiles for selected species')
    ...
    # Add a custom legend entry explaining the labels
    handles, labels = ax.get_legend_handles_labels()
    handles.insert(0, plt.Line2D([], [], color='none', label='(Exp., Species)'))
    labels.insert(0, '(Exp., Species)')
    
    # Adjust legend placement based on user preference
    if show_legend:
        if legend_outside:
            ...
            legend_width_scaled = legend_width / fig_width_display
            plt.subplots_adjust(right=1 - legend_width_scaled)
            plt.legend(handles, labels, loc='center left', bbox_to_anchor=(1, 0.8))
            fig_size = plt.gcf().get_size_inches()
            fig_size[0] = fig_size[0] + fig_size[0] * legend_width_scaled
            plt.gcf().set_size_inches(fig_size)
        else:
...
def plot_data_MB(data, species, stochiometry, scaler=1, ylim=None, t_unit=None, y_unit='mM', fig_size_scaler=1,
                 plot_mode='together',legend_outside=False,DP_scaler=0.35,linewidth=0.7,show_legend=True):
    """
    Inputs the kinetic data "data" that has been imported from an excel file using Pd.read_excel() \
    as a dictionary of DataFrames containing the concentration profiles for each different excess \
    ...
    Args:
        ...
        value of 6. 
        linewidth (float, int): The linewidth parameter used to generate plots. Is by default set to 1.
        show_legend: Set to True to show the legend. False to not show any legends. 
    """
    # Make the plot mode 'separate' if there is only one experiment in the dataset. 
    if len(list(data.keys()))==1:
        plot_mode='separate'
    ...
    if plot_mode=='together':
        ...
        class ScrollablePlot:
            def __init__(self, data_dict,species, stochiometry,y_unit='M',t_unit=None,fig_size_scaler=1,
                         ylim=None,scaler=1,legend_outside=False,DP_scaler=0.35,linewidth=0.7,show_legend=True):
                data_1=[]
        ...
        t_unit=t_unit,y_unit=y_unit,scaler=scaler,ylim=ylim,legend_outside=legend_outside,linewidth=linewidth, DP_scaler=DP_scaler,show_legend=show_legend)
        plt.show()
    else: 
