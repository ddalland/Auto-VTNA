...
class Normal_VTNA:
    ...
    def plot_VTNA(self,y_unit='M',size_scaler=1,title=None,grid=False, \
                  ...
                  linewidth=0.7,scale_xaxis=False,xaxis_notation='Automatic',
                   title_fontsize=10,order_rounding=3,standard_form_label_position=1):
        """
        ...
        if scale_xaxis:
            ...
            scaler=max(tTs)
        # Loop through each experiment in the result from the normal VTNA calculation. Add the smallest y value for each exp.
        min_ys=[]
        ...
        for i in range(len(orders)):
            x_label = x_label + f"[{normalised_species[i]}]^{{{float(orders[i]):.{int(order_rounding)}f}}}"
        x_label=x_label+"\u0394t$."
        ...
        if title==None:
            ...
            title_string=title
        # Check if the title fontsize argument is valid. Set to 10 if it isn't.
        try:
            title_fontsize_2 = int(title_fontsize)
        ...
        if standard_form_label_position:
            ax = plt.gca()
            offset_text = ax.xaxis.get_offset_text()
            #offset_text.set_transform(ax.transAxes)
            offset_text.set_position((standard_form_label_position, 0))  # move above the axis
    ...
    def plot_VTNA_with_overlay_score(self,y_unit='M',size_scaler=1,title=None,grid=False,\
                                     xaxis_size_scaler=None,DP_scaler=0.35,fit_metric='RMSE',\
        ...
        Args: 
            ...
            xaxis_notation (string): Determines the notation of the x axis. Can be set to 'Normal' to avoid scientific \
            notation, 'Scientific' to ensure scienfific notation eg. 1e2 instead of 100, or 'Automatic' to get the default setting.\
            return_slope (bol): Decides whether to return the slope of the fitted line if deg=1. If deg is not 1, only image is returned. 
        """
        plt.rcParams['font.family'] = 'DejaVu Sans'
        ...
        if not show_fit_function and not show_overlay_score:
            fig = self.plot_VTNA(y_unit=y_unit,size_scaler=size_scaler,title=title,grid=grid,xaxis_size_scaler=xaxis_size_scaler,
            ...
            xaxis_notation=xaxis_notation, order_rounding=order_rounding)
            return fig
        ...
        x_label="$\u03A3"
        for i in range(len(orders)):
            x_label = x_label + f"[{normalised_species[i]}]^{{{float(orders[i]):.{order_rounding}f}}}"
        x_label=x_label+"\u0394t$."
        # Define the x label. The size of the xlabel is scaled with "xaxis_size_scaler" if defined. 
        if xaxis_size_scaler!=None:
        ...
        Data_points_y=[]
        Data_points_t=[]
        # Store the lowest scaled (and possibly shifted) y value for each experiment:
        min_ys=[]
        ...
        if deg==1:
            ...
            order_of_magnitude_difference = round(abs(np.log10(abs(slope)) - np.log10(abs(std_err_slope))))

            # Format the slope and standard error using scientific notation
            slope_formatted = f'{slope:.3e}'
            std_err_slope_formatted = f'{std_err_slope:.{4 - order_of_magnitude_difference}e}'

            # Create the final string
            slope_string = f'\nSlope (non-scaled tT and y): {slope_formatted} ± {std_err_slope_formatted}'

        # Check if the inputted title_fontsize can be converted to an integer
        try:
            title_fontsize_2 = int(title_fontsize)
        except (ValueError, TypeError):
            title_fontsize_2 = 10
        plt.title(title_string+GOF_string+slope_string,fontsize=title_fontsize_2)
        ...
        if plt_show:
            plt.show()
        if deg==1 and return_slope:
            return fig, slope, std_err_slope
        return fig