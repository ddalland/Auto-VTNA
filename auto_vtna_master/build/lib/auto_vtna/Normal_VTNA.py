from auto_vtna.VTNA_functions import VTNA_new,data_scaling_y,score_fit,origin_line
import numpy as np
...
class Normal_VTNA:
    def __init__(self,data,VTNA_selection):
        ...
        if list(self.result.values())[0][VTNA_selection['output_species']].to_numpy()[0]>0:
    ...
    def plot_VTNA(self,y_unit='M',size_scaler=1,title=None,grid=False, \
        ...
        for exp in self.result.keys():
            ...
            x_values=[float(j) for j in self.result[exp]['tT'].to_numpy()]
            ys=[float(j) for j in self.result[exp][output_species].to_numpy()]
            plt.plot(x_values,ys,marker='o',label=exp,markersize=6*DP_scaler)
        if show_legend:
            if legend_outside:
        ...
        # Add shifted to the y axis label if a reactant has been used as the output species.
    ...
    def plot_VTNA_with_overlay_score(self,y_unit='M',size_scaler=1,title=None,grid=False,\
        ...
        if not show_fit_function and not show_overlay_score:
            self.plot_VTNA(y_unit=y_unit,size_scaler=size_scaler,title=title,grid=grid,xaxis_size_scaler=xaxis_size_scaler,DP_scaler=DP_scaler,show_legend=show_legend,legend_outside=legend_outside,xtick_rotation=xtick_rotation)
            return
        ...
        for i in tables_scaled.keys():
            ...
            ax.plot(x_values,ys,marker='o',label=i,markersize=6*DP_scaler)
        ...
        if list(tables_scaled.values())[0][output_species].to_numpy()[0]>0:
            y_label=y_label+' (Shifted)'
        ax.set_ylabel(f'Scaled [{output_species}]')
