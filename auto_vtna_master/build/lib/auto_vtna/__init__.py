    ...
    if plot_mode=='together':
        ...
        class ScrollablePlot:
            def __init__(self, data_dict, ylim=None, y_unit='M', t_unit=None, fig_size_scaler=1,legend_outside=legend_outside,DP_scaler=1):
                ...
                self.fig.text(0.0, 0.003, note_text, ha='left', color='gray')
    ...
    for i in data.keys():
        ...
        for j in data[i].keys():
            if j==t or j not in species:
                continue
            ax.plot(time,data[i][j],label=f'{i},{species}',linestyle='-',marker='o',markersize=6*DP_scaler)
    ...
    if plot_mode == 'together':
        ...
        class ScrollablePlot:
            def __init__(self, data_dict,species, stochiometry,y_unit='M',t_unit=None,fig_size_scaler=1,ylim=None,scaler=1,legend_outside=False):
                ...
                # Create a note to inform user of scrolling feature.
                note_text = 'NB: Use left or right arrows to scroll to other experiments.'
                self.fig.text(0.2, 0.003, note_text, ha='center', color='gray')
