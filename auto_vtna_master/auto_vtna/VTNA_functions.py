...
def VTNA_orders(data,VTNA_selection,order_range=[-1.5,2.5],resolution=8,deg=5,fit_metric='RMSE',iterations=6,constraint='monotonic',score_interval=0.15,fixed_order_species=None):
        """
    ...
    number of normalised reaction species in the VTNA selection dictionary.")
        ...
        orders=np.linspace(order_range[0],order_range[1],round(resolution*1.5))
        ...
        # The zoom factor is defined as 140% of the differnce between adjacent orders in the first reaction order grid applied. 
        range_span=abs(order_range[-1]-order_range[0])
        mesh_resolution=abs(orders[0]-orders[1])
        multiplier=1.4
        if resolution==4 or resolution==5:
            multiplier=1.2
        zoom_factor=mesh_resolution*multiplier
        ...
        return pd.DataFrame(result_dict),best_orders_3,pd.DataFrame(interval_dict)
...
def explore_orders(dt_lists,output_lists,dNS_lists,dimensions,orders,fit_metric,deg,constraint,fit_direction):
    ...
    for i in order_pairs:
        ...
        if constraint=='origin':
            if deg==1:
                params = curve_fit(origin_line, tT, y)
                a=params[0]
                y_pred=a*tT
            else:
        ...
        if constraint=='monotonic': 
            ...
            FQs.append(FQ)
        if constraint==None:
            polyfit=np.polyfit(tT,y,deg=deg)
            pred_y=np.polyval(polyfit,tT)
            FQ=score_fit(y,pred_y,fit_metric)
            FQs.append(FQ)
...
def on_click(event,VTNA_selection,data,y_unit,size_scaler,fit_metric,deg,constraint,fixed_order_species=None,axis_species=None,show_legend=True):
    ...
    if event.button==3 or event.button==2:
        ...
        if list(tables.values())[0][output_species].to_numpy()[0]>0:
            y_label=y_label+' (Shifted)'
        ax2.set_ylabel(f'Scaled [{output_species}]')
        if constraint=='monotonic':
        ...
        if str(orders[-1])[0]=='-':
            w=1+w
        ax2.set_xlabel(x_label)
        ax2.set_ylabel(f'Concentration in {y_unit}')
